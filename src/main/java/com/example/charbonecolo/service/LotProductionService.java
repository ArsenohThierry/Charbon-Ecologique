...
public class LotProductionService {
    ...
    // Statut actuellement en cours pour un lot (celui dont date_fin est NULL,
    // i.e. pas encore clôturé). Si tous sont clôturés, retourne le plus récent.
    public Optional<StatutsLotProductionModel> getStatutActuel(LotProductionModel lot) {
        List<StatutsLotProductionModel> actifs = statutsLotProductionRepository.findActiveByLotProduction(lot);
        if (!actifs.isEmpty()) return Optional.of(actifs.get(0));
    ...
    public void validerFinStatutCourant(Integer idLotProduction, LocalDateTime dateFin) {
        ...
        if (dateFin.toLocalDate().isBefore(statutCourant.getDateStatut().toLocalDate())) {
            throw new BusinessException("La date de fin doit être postérieure ou égale à la date de début du statut en cours ("
