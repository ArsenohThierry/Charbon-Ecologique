...
public class PaiementService {
    ...
    public void initDonnees() {
        ...
        if (count == 0) {
            jdbcTemplate.update("INSERT INTO paiement_statuts (libelle) VALUES ('Non payée'), ('Payée'), ('Partiellement payée')");
        }
        count = jdbcTemplate.queryForObject("SELECT COUNT(*) FROM methode_paiement", Integer.class);
        if (count == 0) {
            jdbcTemplate.update("INSERT INTO methode_paiement (libelle) VALUES ('Espèces'), ('Mobile money'), ('Carte bancaire'), ('Virement')");
        }
        Integer existe = jdbcTemplate.queryForObject("SELECT COUNT(*) FROM commande_statuts WHERE id = 6", Integer.class);
        if (existe == 0) {
    ...
    public Map<String, Object> creerFacture(Integer commandeId, BigDecimal fraisLivraison, Integer methodePaiementId) {
        ...
        jdbcTemplate.update("""
            INSERT INTO facture_detail (id_facture, montant, libelle)
            VALUES ((SELECT id FROM facture WHERE reference = ?), ?, 'Frais de livraison')
        """, refFacture, fraisLivraison);

        Integer existeStatut = jdbcTemplate.queryForObject("SELECT COUNT(*) FROM commande_statuts WHERE id = 6", Integer.class);
        if (existeStatut == 0) {
            jdbcTemplate.update("INSERT INTO commande_statuts (id, libelle) VALUES (6, 'payee')");
        }
